...
export const CHANGELOG: ChangelogEntry[] = [
    {
        ...
        items: [
            ...
            { type: "improvement", text: "Change the owner, phone number and email (a company's owner, phone number and website) right where they show under the name: click the value, or the pencil next to a phone number, email or website, which still open when you click them. Change a contact's company with the pencil next to it under the name. On a phone, tap them in the key facts at the top of Activity." },
            { type: "improvement", text: "About this contact (or company) no longer repeats what is already at the top of the page. The email, phone number, company and owner (a company's phone number, website and owner) only appear there while they are empty, so you can still fill them in." },
            { type: "improvement", text: "The Email button at the top of a contact's page now reads Send email, and the kinds above the box read Log call, Log meeting and Log email, so it is clear which buttons reach the person and which record something that already happened. On a phone the buttons under the name still say Call and Email." },
            { type: "improvement", text: "Each row in History now says what happened in its title, for example \"Call · Connected\" or \"Email · Proposal v2\", then what was written, and under it, in small letters, who did it and when, for example \"Hanung Prasetyo · 2 hours ago\". Before, the name came right after the title and could be read as the result of the call. Upcoming follow-ups read the same way: the title, the notes, then when it is due and who it is for." },
            { type: "improvement", text: "In History, the filter buttons (All, Notes, Calls…) now sit on the same line as the title on a computer, with a single line under them. When they don't all fit, swipe or scroll them sideways. On a phone they are under the title." },
            { type: "feature", text: "The first time you open a contact or a company, About shows \"Click a value to change it.\" (on a phone, \"Tap a value to change it.\"). Close it with ✕ and it stays closed on every device. You only see it if you are allowed to edit the record." },
            { type: "improvement", text: "Note, Log call, Log meeting, Log email and Follow-up above the box are now one connected row of buttons, each with its icon; the one you picked is shaded blue, with a tick in place of its icon. They no longer look like the filters in History, which only choose what the list shows. When the box is narrow (a small laptop screen, or a phone) the buttons show only their icons, the one you picked keeping its own icon: point at one to see its name. On a phone, the title of the panel says what you are logging, for example \"Log call\"." },
        ],
    },
    {
        date: "2026-09-25",
