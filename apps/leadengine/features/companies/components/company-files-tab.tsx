...
const BUCKET = "company_attachments"
...
export function CompanyFilesTab({ companyId }: CompanyFilesTabProps) {
    ...
    const [files, setFiles] = useState<CompanyAttachmentRow[]>([])
    const [loading, setLoading] = useState(true)
    const [uploading, setUploading] = useState(false)
    const [dragOver, setDragOver] = useState(false)
    const [pendingDelete, setPendingDelete] = useState<CompanyAttachmentRow | null>(null)
    ...
    const fetchFiles = useCallback(async () => {
        ...
        if (error) {
            console.error("[CompanyFilesTab] fetch error:", error.message)
        } else {
            setFiles(data ?? [])
        }
        setLoading(false)
    ...
    const uploadOne = useCallback(
        async (file: File): Promise<boolean> => {
            ...
            const { error: rowErr } = await supabase.from("company_attachments").insert({
    ...
    const handleDelete = useCallback(
        async (file: CompanyAttachmentRow) => {
            setDeleting(true)
            const { error: storageErr } = await supabase.storage
                .from(BUCKET)
                .remove([file.storage_path])
            if (storageErr) {
                console.warn("[CompanyFilesTab] storage delete error:", storageErr.message)
            }
                ...
                {loading ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-14 text-center px-6">
                        ...
                        <p className="text-[12px] text-slate-400 max-w-xs mb-4">
                            Drag &amp; drop or click <span className="font-medium">Upload</span> to attach contracts,
                            proposals, and supporting documents (max {MAX_FILE_SIZE_MB} MB each).
                        </p>
                        <Button
                            size="sm"
                            variant="outline"
                ...
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            {pendingDelete
                                ? <><span className="font-medium text-slate-700">{pendingDelete.file_name}</span> will be permanently removed from this company. This cannot be undone.</>
                                : null}
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
