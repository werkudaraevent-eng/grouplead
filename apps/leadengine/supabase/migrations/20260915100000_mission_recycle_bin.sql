-- A recycle bin for missions.
--
-- Deleting a mission marks it rather than removing it; an admin restores it
-- or removes it for good, and anything older than the retention period is
-- purged when the bin is opened. Every read of the missions table in the app
-- excludes marked rows; the two SQL functions the list uses do the same here.
...
CREATE INDEX IF NOT EXISTS missions_deleted_idx
  ON sales_mission.missions (company_id, deleted_at DESC)
