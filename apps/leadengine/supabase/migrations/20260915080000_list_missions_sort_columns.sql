-- Every column of the mission table sorts, not only the schedule.
--
-- p_sort takes "upcoming" (the default: what is ahead first, nearest at the
-- top, then the past most recent first) or "<column>:<asc|desc>" for
-- schedule, client, location, sales (the primary's name) and status (the
-- visit's lifecycle: upcoming, awaiting a report, draft, reported,
-- unscheduled, cancelled). The older bare "asc"/"desc" still mean the
-- schedule. Same signature as before, so callers need no change.
...
AS $$
  ...
  ORDER BY
    ...
    -- Inside any group the schedule keeps its meaning, then newest first.
