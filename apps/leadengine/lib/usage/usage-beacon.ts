 ...
 * run without a DOM so they can be tested. The same rules as Sales
 * Activity's beacon, so the two apps count a person the same way.
 *
 ...
 *   in the next call, so a quick Pipeline → lead → contact is three opens
 *   in one request, not three requests or one lost page.
 ...
 *   heartbeat: "seen", no page opened. Someone working the Pipeline all
