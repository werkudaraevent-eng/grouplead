...
import { OWNER_EMAIL_COLUMN, PROSPECT_COLUMNS } from "@/lib/prospects/prospect-io"

export const dynamic = "force-dynamic"

/** The prospect import template. Guarded on prospect create: it lists the team's emails. */
export async function GET() {
  ...
  const sheet = XLSX.utils.aoa_to_sheet([
    PROSPECT_COLUMNS.map((column) => (column.required ? `${column.header} *` : column.header)),
    PROSPECT_COLUMNS.map((column) => column.example),
  ])
  sheet["!cols"] = PROSPECT_COLUMNS.map((column) => ({ wch: Math.max(column.header.length + 2, column.example.length, 18) }))
  XLSX.utils.book_append_sheet(book, sheet, "Prospek")

  const optionRows: string[][] = [["Kolom", "Nilai yang diterima"]]
  for (const salutation of salutations) optionRows.push(["Sapaan", salutation])
  optionRows.push([])
  ...
  const guide: string[][] = [
    ...
    ["  Hanya Perusahaan yang wajib. Kosongkan sel yang tidak Anda punya."],
    [""],
    ["TELEPON"],
