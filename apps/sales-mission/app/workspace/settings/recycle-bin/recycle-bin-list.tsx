...
import { emptyRecycleBin, purgeMissions, restoreMissions } from "@/app/actions/recycle-bin-actions"
import type { DeletedMission } from "@/lib/missions/recycle-bin-queries"
...
/**
 * The bin as a data table with a selection bar, the same shape as the
 * mission list: pick rows, then act on them. Restore is the safe action and
 * needs no confirmation; removing for good does, in a dialog that names the
 * count, because that is the one step here that cannot be undone.
 */
export function RecycleBinList({ items, now }: { items: DeletedMission[]; now: string }) {
        ...
        {chosen.length > 0 ? (
          <>
            ...
            <span className="ml-auto flex items-center gap-2">
              ...
              <Button size="sm" variant="ghost" onClick={() => setChosen([])} aria-label="Batalkan pilihan">
              ...
              {items.map((item) => {
                ...
                return (
                  <TableRow key={item.id} data-state={chosen.includes(item.id) ? "selected" : undefined}>
                    <TableCell>
                      <Checkbox checked={chosen.includes(item.id)} onCheckedChange={(value) => toggle(item.id, value === true)} aria-label={`Pilih ${item.clientCompanyName}`} />
                    </TableCell>
                    <TableCell>
                      <span className="block font-semibold text-foreground">{item.clientCompanyName}</span>
                      <span className="block text-xs text-muted-foreground">{[item.missionType, item.location].filter(Boolean).join(" · ")}</span>
                    </TableCell>
                    <TableCell className="text-sm text-foreground">{item.scheduledStart ? formatMissionSchedule(item.scheduledStart, nowDate) : "—"}</TableCell>
                    <TableCell>
                      <span className="block text-sm text-foreground">{stamp(item.deletedAt)}</span>
                      <span className="block text-xs text-muted-foreground">{item.deletedByName ?? "—"}</span>
                    </TableCell>
                    <TableCell className={cn("text-right text-sm tabular-nums", left <= 3 ? "text-[var(--danger-foreground)]" : "text-muted-foreground")}>
                      {left === 0 ? "hari ini" : `${left} hari`}
                    </TableCell>
                  </TableRow>
                )
              })}
