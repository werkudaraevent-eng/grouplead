...
/**
 * Per orang: everyone who can use the app, most recently seen first. A
 * table on a desk, cards on a phone (the list recipe the other lists use).
 * Nothing links: a person's usage is these four numbers, not a trail.
 */
...
export function UsageTrend({ weekly, className }: { weekly: number[]; className?: string }) {
  ...
  const label = `Hari aktif per minggu, ${TREND_WEEKS} minggu terakhir: ${weekly.join(", ")}. Minggu ini ${last} hari.`
  return (
    <svg viewBox={`0 0 ${width} ${height}`} width={width} height={height} role="img" aria-label={label} className={cn("shrink-0 overflow-visible text-chart-1", className)}>
      <title>{label}</title>
...
function IdleChip() {
  return <span className="inline-flex shrink-0 items-center rounded-md bg-muted px-1.5 py-0.5 text-[11px] font-medium text-muted-foreground">Tidak aktif 7 hari</span>
}

function LastSeen({ person, now }: { person: PersonUsage; now: Date }) {
  if (!person.lastSeenAt) return <span className="text-muted-foreground">Belum pernah</span>
  return (
...
export function UsagePeople({ people, now }: { people: PersonUsage[]; now: Date }) {
  if (people.length === 0) {
    return <p className="px-5 py-6 text-sm text-muted-foreground">Belum ada orang dengan akses Sales Activity di unit ini.</p>
  }
              ...
              {person.state !== "never" && (
                <div className="mt-2 flex items-center justify-between gap-3">
                  <p className="text-xs text-muted-foreground">
                    <span className="font-medium tabular-nums text-foreground">{number.format(person.daysActive30)}</span> hari aktif (30 hari) ·{" "}
                    <span className="font-medium tabular-nums text-foreground">{number.format(person.views7)}</span> pembukaan (7 hari)
                  </p>
          ...
          <Table className="min-w-[680px] table-fixed text-sm">
            ...
            <TableHeader>
              <TableRow className="hover:bg-transparent">
                ...
                <TableHead className="text-right" title="Berapa hari berbeda orang ini membuka aplikasi">
                  Hari aktif<Window>30 hari</Window>
                </TableHead>
                <TableHead className="text-right" title="Berapa halaman yang dibuka orang ini">
                  Pembukaan<Window>7 hari</Window>
                </TableHead>
                <TableHead title="Hari aktif per minggu">
                  Tren<Window>{TREND_WEEKS} minggu</Window>
                </TableHead>
                <TableHead className="hidden xl:table-cell">Halaman terakhir</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {people.map((person) => {
                  ...
                  <TableRow key={person.id}>
                    <TableCell>
                      <span className="flex min-w-0 items-center gap-2">
                        ...
                        <span className="min-w-0 truncate font-medium text-foreground" title={person.name}>{person.name}</span>
                        {person.state === "idle" && <IdleChip />}
                      </span>
                    </TableCell>
                    ...
                    <TableCell className="text-right tabular-nums">{person.state === "never" ? <span className="text-muted-foreground">—</span> : number.format(person.daysActive30)}</TableCell>
                    <TableCell className="text-right tabular-nums">{person.state === "never" ? <span className="text-muted-foreground">—</span> : number.format(person.views7)}</TableCell>
                    <TableCell>
                      <UsageTrend weekly={person.weekly} />
                    </TableCell>
