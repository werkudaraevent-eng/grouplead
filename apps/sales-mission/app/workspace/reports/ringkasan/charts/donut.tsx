...
/**
 * A donut with the total in the hole and its legend as a list with
 * shares, because a slice without its number is a guess. The list sits
 * beside the ring when the cell is wide, under it when it is not.
 */
export function Donut({ slices, total, unit }: { slices: Array<{ key: string; label: string; value: number; share: number; color: string }>; total: number; unit: Unit }) {
      ...
      <div className="relative min-h-0">
        <ResponsiveContainer width="100%" height="100%">
          <PieChart accessibilityLayer>
            <Pie data={slices} dataKey="value" nameKey="label" innerRadius="62%" outerRadius="90%" paddingAngle={1.5} stroke="var(--card)" strokeWidth={2} isAnimationActive={false}>
              {slices.map((slice) => (
