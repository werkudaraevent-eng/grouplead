...
/**
 * Tells Pengaturan → Pemakaian that this person is here, and on which kind
 * of page. Draws nothing. The rules (a call at most every 30 seconds with
 * the opens batched, nothing while the tab is hidden, a heartbeat after
 * five active minutes on one page) live in `lib/usage/usage-beacon.ts`;
 * this only wires them to the router and the document.
 *
 * Only the route is sent (`/workspace/activities/:id`), never an id, a
 * query string, or anything typed. A failed call is dropped: usage must
 * never cost a page anything.
 */
