...
/**
 * A page's description, shown until the person closes it.
 *
 * The sentence under a page's title teaches (what the page is for, whose
 * rows you see, what a yellow edge means). A newcomer needs it once; on
 * every later visit it is two lines between the title and the work. M3
 * keeps supporting text that has done its job out of the way, and the
 ...
 * phone and laptop alike. The key is per page: `list-intro-<list>` for the
 * three lists, `page-intro-<page>` for every other page (`lib/hints/hint-key.ts`).
 */
export function PageIntro({ hintKey, children, className }: { hintKey: string; children: React.ReactNode; className?: string }) {
