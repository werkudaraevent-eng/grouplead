...
describe("describeAudit", () => {
  it("names the mission on create and delete", () => {
    expect(describeAudit(row({ action: "INSERT", changes: { to: {} } })).sentence).toBe("membuat mission ke PT Arunika")
    expect(describeAudit(row({ action: "DELETE", changes: { from: {} } })).tone).toBe("delete")
  })

  it("reads a status change as the verb it is", () => {
    expect(describeAudit(row({ changes: { status: { from: "ACCEPTED", to: "CANCELLED" } } })).sentence).toBe("membatalkan mission ke PT Arunika")
  ...
  it("labels changed fields for the detail view and hides plumbing columns", () => {
    ...
    expect(result.sentence).toBe("mengubah lokasi pada mission ke PT Arunika")
  })
  ...
  it("formats timestamps in mission time and booleans in words", () => {
    ...
    expect(result.sentence).toBe("memindahkan jadwal mission ke PT Arunika")
    ...
    expect(flag.sentence).toBe("mengubah aturan mission: sales harus mengonfirmasi tidak → ya")
  })

  it("reads an assignment answer", () => {
    expect(describeAudit(row({ tableName: "assignments", changes: { response: { from: "PENDING", to: "ACCEPTED" } } })).sentence).toBe("menerima penugasan mission ke PT Arunika")
    expect(describeAudit(row({ tableName: "assignments", action: "INSERT", changes: { to: { assignment_role: "PRIMARY" } } })).sentence).toBe("ditugaskan sebagai sales utama pada mission ke PT Arunika")
  })
