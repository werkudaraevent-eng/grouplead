...
describe("collapseVisits", () => {
  it("counts each path once, in the order of its last open", () => {
    expect(collapseVisits(["/workspace", "/workspace/activities", "/workspace"])).toEqual([
      { path: "/workspace/activities", views: 1 },
      { path: "/workspace", views: 2 },
    ])
  })
...
describe("createUsageBeacon", () => {
  it("sends the first page at once", () => {
    const h = harness()
    h.beacon.visit("/workspace")
    expect(h.sent).toEqual([[{ path: "/workspace", views: 1 }]])
  })
  ...
  it("calls the server at most once per 30 seconds and loses no page opened in between", () => {
    ...
    h.beacon.visit("/workspace")
    h.advance(5_000)
    h.beacon.visit("/workspace/activities")
    h.advance(5_000)
    h.beacon.visit("/workspace/activities/:id")
    ...
    expect(h.sent[1]).toEqual([
      { path: "/workspace/activities", views: 1 },
      { path: "/workspace/activities/:id", views: 1 },
    ])
  })
  ...
  it("sends a page opened after the window has passed straight away", () => {
    ...
    h.beacon.visit("/workspace/calendar")
    expect(h.sent).toHaveLength(2)
  })
  ...
  it("sends a heartbeat after five minutes on one page, only if the person did something", () => {
    ...
    h.beacon.interact()
    h.beacon.tick()
    expect(h.sent[1]).toEqual([{ path: "/workspace", views: 0 }])
  })
  ...
  it("keeps a runaway burst bounded", () => {
    const h = harness()
    h.beacon.visit("/workspace")
    for (let index = 0; index < USAGE_MAX_PENDING + 20; index += 1) h.beacon.visit("/workspace/activities")
    h.advance(USAGE_MIN_INTERVAL_MS)
...
describe("usageVisitsSchema", () => {
  it("accepts opens and a heartbeat under /workspace", () => {
    expect(usageVisitsSchema.safeParse([{ path: "/workspace", views: 1 }]).success).toBe(true)
    expect(usageVisitsSchema.safeParse([{ path: "/workspace/activities/:id", views: 0 }]).success).toBe(true)
  })
  it("refuses paths outside the app, long paths, odd counts and empty batches", () => {
    expect(usageVisitsSchema.safeParse([{ path: "/login", views: 1 }]).success).toBe(false)
    expect(usageVisitsSchema.safeParse([{ path: "https://example.com/workspace", views: 1 }]).success).toBe(false)
    expect(usageVisitsSchema.safeParse([{ path: `/workspace/${"a".repeat(200)}`, views: 1 }]).success).toBe(false)
    expect(usageVisitsSchema.safeParse([{ path: "/workspace", views: -1 }]).success).toBe(false)
    expect(usageVisitsSchema.safeParse([{ path: "/workspace", views: 1.5 }]).success).toBe(false)
