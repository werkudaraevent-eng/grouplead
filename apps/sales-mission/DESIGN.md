...
| Settings matrix layout | List-detail: a fixed-width list that stays put, a detail pane that takes the rest; the page is the only scroller; a table row is a list item (headline, one supporting line, the rule behind an info icon), secondary actions in a trailing overflow menu (M3 list-detail canonical layout, list item, data table; HubSpot Roles) | Role & Izin: 264px role list pinned with `sticky`, fixed-layout table whose Modul column takes what the switches leave, presets in a per-row menu, the reading guide folded under one button |
