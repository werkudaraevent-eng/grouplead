...
| Wall board | Built like a departures board, not a dashboard: one dominant region (the schedule), rows tall enough for four metres, the time in large tabular figures at the left of every row, status as a coloured edge and a word, the visit happening now (or the next) tinted in its place in the list, a small clock, one summary sentence instead of KPI tiles, no admin text on a public screen, and no pagination: a list taller than its box scrolls on its own and returns to the top. Everything sized in `em` from one root that follows the screen width, so 1080p and 4K show the same composition (airport FIDS, meeting-room displays; M3 large screens, reduced motion) | `app/board/board-view.tsx` with `fontSize: clamp(15px, 1.05vw, 44px)`, `AutoScroll`, `BoardClock`, `BoardTeamNext` fields |
