...
import {
    ...
    ShieldCheck, LogOut, ChevronLeft, Settings, Loader2, GitBranchPlus,
} from "lucide-react"
...
const adminNav = [
    { href: "/settings/pipeline", label: "Pipeline Settings", icon: GitBranchPlus },
    { href: "/settings/companies", label: "Company Management", icon: Building2 },
    ...
    return (
        <div className="flex flex-col h-full bg-sidebar text-sidebar-foreground">
            ...
            <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
                <p className="px-3 mb-2 text-[10px] font-bold uppercase tracking-widest text-sidebar-foreground/40">Menu</p>
                {mainNav.map((item) => {
                ...
                <PermissionGate resource="companies" action="read">
                    <p className="px-3 mb-2 text-[10px] font-bold uppercase tracking-widest text-sidebar-foreground/40">Administration</p>
                    {adminNav.map((item) => {
            ...
            <div className="border-t border-sidebar-border px-3 py-3 shrink-0">
                <div className="flex items-center gap-3 px-2 py-2 rounded-lg hover:bg-sidebar-accent/50 transition-colors">
                    ...
                    <div className="flex-1 min-w-0">
                        <p className="text-sm font-semibold truncate leading-tight">{profile?.full_name || "Loading..."}</p>
                        <p className="text-[11px] text-sidebar-foreground/50 truncate">{getRoleLabel(profile?.role ?? null)}</p>
                    </div>
                    <Button variant="ghost" size="icon" className="h-8 w-8 text-sidebar-foreground/30 hover:text-sidebar-foreground shrink-0" onClick={handleLogout} disabled={loggingOut}>
