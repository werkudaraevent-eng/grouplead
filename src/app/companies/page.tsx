    ...
    return (
        <div className="w-full h-[calc(100vh-64px)] sm:h-full py-6 flex flex-col overflow-hidden">
            <div className="mb-6 shrink-0 px-4 sm:px-6 lg:px-8">
                <h1 className="text-2xl font-extrabold text-foreground tracking-tight">Companies</h1>
                <p className="text-sm text-muted-foreground mt-1">Manage your client organizations and their details.</p>
            </div>
                    ...
                    <Popover>
                        ...
                        <PopoverContent align="end" className="w-72 p-0">
                            <div className="px-4 py-3 border-b border-slate-100 flex items-center justify-between">
                                ...
                                <button onClick={() => { setColumns(DEFAULT_COLUMNS); localStorage.removeItem("companies_cols_order") }} className="text-[12px] text-blue-600 hover:text-blue-700 flex items-center gap-1 font-medium">
                                        ...
                                        return (
                                            <TableHead key={col.id} className={className} style={style}>
                                                <div className="truncate w-full">
                                                    <button onClick={() => handleSort(col.id)} className="flex items-center gap-1.5 hover:text-slate-900 transition-colors w-full uppercase">
                                ...
                                {paginatedData.length === 0 ? (
                                    ...
                                    paginatedData.map((company, idx) => (
                                        <TableRow
                                            ...
                                            className={`${selectedIds.has(company.id) ? "" : ""} transition-colors cursor-pointer group`}
                                        >
                                            <TableCell className={`px-4 py-2 text-center align-middle sticky left-0 z-20 shadow-[1px_0_0_0_#e2e8f0] transition-colors ${selectedIds.has(company.id) ? "bg-[#E0F2FE] group-hover:bg-[#d0e8fc]" : "bg-white group-hover:bg-[#F0F9FF]"}`} onClick={(e) => e.stopPropagation()}>
                                                <Checkbox 
                                            ...
                                            <TableCell className={`px-2 py-2 text-center align-middle text-[12px] text-slate-400 font-medium sticky left-[40px] z-20 shadow-[1px_0_0_0_#e2e8f0] transition-colors ${selectedIds.has(company.id) ? "bg-[#E0F2FE] group-hover:bg-[#d0e8fc]" : "bg-white group-hover:bg-[#F0F9FF]"}`}>
                                            ...
                                            <TableCell className={`px-4 py-2 align-middle text-right sticky right-0 z-20 shadow-[-1px_0_0_0_#e2e8f0] transition-colors ${selectedIds.has(company.id) ? "bg-[#E0F2FE] group-hover:bg-[#d0e8fc]" : "bg-white group-hover:bg-[#F0F9FF]"}`} onClick={(e) => e.stopPropagation()}>
                                                <DropdownMenu>
