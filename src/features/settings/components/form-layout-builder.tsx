...
export function FormLayoutBuilder({ companyId, customSchemas, onEditCustomField }: FormLayoutBuilderProps) {
    ...
    useEffect(() => {
        const fetchLayout = async () => {
            ...
            if (configOption) {
                ...
                try {
                    ...
                    if (parsed.tabs && parsed.requiredOverrides) {
                        ...
                        if (parsed.tabSettings) loadedTabSettings = parsed.tabSettings
            ...
            // Self-heal: ensure tab settings exist
    ...
    const saveLayout = async () => {
        setIsSaving(true)
        const payload = JSON.stringify({ tabs: items, requiredOverrides, visibilityRules, tabSettings })
        if (layoutConfigId) {
        ...
        <div className="flex justify-between items-center bg-white p-3 rounded-lg border shadow-sm">
            ...
            <Button size="sm" variant="outline" className="h-8 gap-1.5" onClick={() => {
                ...
                if (label) {
                    ...
                    setTabSettings(prev => {
                        const maxSortInfo = Object.values(prev).reduce((max, s) => Math.max(max, s.sortOrder), -1);
                        return { ...prev, [id]: { label, isHidden: false, sortOrder: maxSortInfo + 1 } };
                    });
                }
            }}>
        ...
        <Card className={`${isHiddenBox ? "border-dashed border-2 bg-muted/40 text-muted-foreground shadow-inner" : "bg-white shadow-sm border-slate-200"} flex flex-col ${heightClass}`}>
            <CardHeader className="py-3 px-4 border-b bg-muted/20 flex-none shrink-0">
                <div className="flex items-center justify-between">
                    <CardTitle className={cn("text-sm flex items-center font-semibold", isTabHidden && "line-through text-muted-foreground")}>
                        {(isHiddenBox || isTabHidden) && <EyeOff className="w-3.5 h-3.5 mr-1.5 text-muted-foreground" />}
                        {title}
                    ...
                    {!isHiddenBox && tabId && (
                        <Popover>
                            <PopoverTrigger asChild>
                                <Button variant="ghost" size="icon" className="h-7 w-7 shrink-0"><Settings className="w-3.5 h-3.5" /></Button>
                            </PopoverTrigger>
                            <PopoverContent className="w-48 p-2" align="end">
                                <div className="space-y-1">
                                    <Button variant="ghost" size="sm" className="w-full justify-start text-xs h-8" onClick={() => {
                                        ...
                                        <Pencil className="w-3.5 h-3.5 mr-2" /> Rename Tab
                                    </Button>
                                    <Button variant="ghost" size="sm" className="w-full justify-start text-xs h-8" onClick={() => {
                                        if (onUpdateTab) onUpdateTab(tabId, { isHidden: !isTabHidden });
                                    }}>
                                        {isTabHidden ? <Eye className="w-3.5 h-3.5 mr-2" /> : <EyeOff className="w-3.5 h-3.5 mr-2" />}
                                        {isTabHidden ? "Show Tab" : "Hide Tab"}
                                    </Button>
                                    <Button variant="ghost" size="sm" className="w-full justify-start text-xs h-8 text-red-600 hover:text-red-700 hover:bg-red-50" onClick={() => onDeleteTab && onDeleteTab(tabId)}>
                                        <Trash2 className="w-3.5 h-3.5 mr-2" /> Delete Tab
                                    </Button>
                                    <div className="h-px bg-slate-100 my-1"></div>
                                    <Button variant="ghost" size="sm" className="w-full justify-start text-xs h-8" onClick={() => {
                                        const newOrder = prompt("Enter new index (0 = first):");
                                        const parsed = parseInt(newOrder || "");
                                        if (!isNaN(parsed) && onUpdateTab) {
                                            onUpdateTab(tabId, { sortOrder: parsed });
                                        }
                                    }}>
                                        Change Sort Index
                                    </Button>
