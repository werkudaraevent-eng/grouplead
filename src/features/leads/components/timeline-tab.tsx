...
const EVENT_CONFIG: Record<EventCategory, EventConfig> = {
    ...
    create:  { icon: Plus,           label: "Deal Created",   nodeBg: "bg-emerald-50", nodeText: "text-emerald-600" },
...
type FilterKey = "all" | "stage" | "note" | "email" | "meeting" | "task"
...
export function TimelineTab({ leadId }: TimelineTabProps) {
    ...
    // ── TASK 1: Relational Data Query ────────────────────────
    const fetchActivities = useCallback(async () => {
        const { data, error } = await supabase
            .from("lead_activities")
            .select("*, profile:profiles!lead_activities_profile_id_fkey(full_name, avatar_url)")
            .eq("lead_id", leadId)
    ...
    const handleLogActivity = async () => {
        ...
        if (error) {
            ...
            setLogType("Note")
            setDialogOpen(false)
            // Re-fetch to show new entry + sync server
            ...
            {filtered.length === 0 && (
                <div className="flex flex-col items-center justify-center py-16 text-center">
                    ...
                    <p className="text-[12px] text-slate-400 max-w-xs">
                        Stage changes, notes, emails, and updates will appear here as they happen.
                    </p>
                </div>
            )}
            ...
            {filtered.length > 0 && (
                <div className="relative w-full pb-10">
                    ...
                    {filtered.map((activity) => {
                        ...
                        // TASK 2: Dynamic user mapping with relational data
                        ...
                        return (
                            ...
                            >
                                ...
                                <div className="flex-1 bg-white border border-slate-200 rounded-lg shadow-sm overflow-hidden flex flex-col hover:shadow-md transition-shadow">
                                    ...
                                    {/* Card Footer (User Info) — TASK 2: Dynamic mapping */}
